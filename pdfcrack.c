...
/* MAX_PASS_LEN is defined in rules.h */
...
atomic_int         g_current_len    = 0;  /* current password length being tested */
long               g_completed_prior = 0; /* completed candidates from prior lengths */

/* ── config (read-only after parse) ──────────────────────────── */
const char        *g_pdf_path = NULL;
const char        *g_charset  = NULL;
...
atomic_long                g_next_idx   __attribute__((aligned(64))) = 0; /* shared work counter for GPU+CPU */
...
int  g_is_brute = 0;  /* 1 = brute-force, 0 = dictionary (for checkpoint) */
...
int g_attack_mode = ATTACK_BRUTE;  /* current attack mode for checkpoint */
int g_auto_phase  = 0;             /* current phase within auto mode */

/* ── prefix/suffix for guided brute-force ─────────────────────── */
char g_prefix[MAX_PASS_LEN + 1] = {0};
char g_suffix[MAX_PASS_LEN + 1] = {0};
int  g_prefix_len = 0;
int  g_suffix_len = 0;
...
int         g_password_mode = PW_MODE_BOTH;
...
IncrHeap        *g_incr_heap = NULL;

/* Forward declarations for incremental save/load (defined later) */
void incr_heap_save(const char *path);
...
int safe_atoi(const char *s, int min_val, int max_val, const char *name)
{
    char *end;
    errno = 0;
...
/* Checkpoint functions are in checkpoint.c / checkpoint.h */
...
/* Rule engine functions (add_rule_1, add_rule_2, parse_rule_op,
 * apply_one_op, apply_rule, load_rules_file, init_rules,
 * rule_dedup_check) are in rules.c / rules.h. */
...
/* Shared double-buffered GPU R6 pipeline driver (companion to run_gpu_sha256) */
static void run_gpu_r6(cand_gen gen, void *ctx, long total, int stride)
{
    ...
    for (int b = 0; b < 2; b++) {
        ...
        pw_storage[b] = malloc((size_t)batch * stride);
        if (!pw_ptrs[b] || !pw_storage[b]) goto done;
    }
...
static void *gpu_r6_brute_worker(void *arg)
{
    GPUBruteArg *a = (GPUBruteArg *)arg;
    BruteCtx bctx = { a->length };
    run_gpu_r6(sha256_brute_gen, &bctx, a->total, MAX_PASS_LEN + 1);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_dict_gen, NULL, g_nwords, MAX_PASS_LEN + 1);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_rule_gen, NULL, g_nwords * g_nrules, MAX_PASS_LEN * 2 + 2);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_hybrid_gen, NULL, g_nwords * g_hybrid_suffix_keyspace, MAX_PASS_LEN * 2 + 2);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_combinator_gen, NULL, g_nwords * g_nwords2, MAX_PASS_LEN * 2 + 2);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_dates_gen, NULL, atomic_load(&g_total), MAX_PASS_LEN + 1);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_mutate_gen, NULL, atomic_load(&g_total), MAX_PASS_LEN * 2 + 2);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_leet_gen, NULL, atomic_load(&g_total), MAX_PASS_LEN + 1);
    free(arg);
    return NULL;
}
...
{
    (void)arg;
    run_gpu_r6(sha256_mask_rule_gen, NULL, g_mask_keyspace * g_nrules, MAX_PASS_LEN * 2 + 2);
    free(arg);
    return NULL;
}
